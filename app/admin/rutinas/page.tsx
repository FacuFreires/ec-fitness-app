...
import { Perfil, Ejercicio, Rutina, RutinaEjercicio } from '@/types/database';
import { Plus, Trash2, X, Loader2, ChevronDown, ChevronUp, Save } from 'lucide-react';
...
function RutinasPageInner() {
    ...
    const [ejerciciosMaster, setEjerciciosMaster] = useState<Ejercicio[]>([]);
    ...
    useEffect(() => {
        Promise.all([
            supabase.from('perfiles').select('*').eq('rol', 'alumno').order('nombre_completo'),
            supabase.from('ejercicios').select('*').order('nombre'),   // tabla: ejercicios
        ]).then(([{ data: a, error: eA }, { data: e, error: eE }]) => {
            if (eA) console.error('[Rutinas] perfiles:', eA.message);
            if (eE) console.error('[Rutinas] ejercicios:', eE.message);
            setAlumnos(a ?? []);
    ...
    useEffect(() => {
        ...
        supabase
            ...
            .then(({ data, error }) => {
                if (error) console.error('[Rutinas] rutinas fetch:', error.message);
                setRutinas(data ?? []);
            });
    }, [selectedAlumno]);
    ...
    const loadRutinaEjercicios = async (rutinaId: string) => {
        ...
        const { data, error } = await supabase
            ...
            .select('*, ejercicio:ejercicios(*)')   // tabla: ejercicios
            .eq('rutina_id', rutinaId)
            .order('orden');
        if (error) console.error('[Rutinas] rutina_ejercicios:', error.message);
    ...
    const createRutina = async () => {
        if (!selectedAlumno || !newRutinaName.trim()) {
            toast.error('Seleccioná un alumno y un nombre');
            return;
        }
        setCreating(true);
        const { data, error } = await supabase
            ...
            .insert({
                ...
                nombre_rutina: newRutinaName.trim(),
                activa: true,               // columna: activa (no activo)
            })
            .select()
            .single();
        if (error) { toast.error('Error al crear'); console.error(error.message); }
        else { setRutinas(r => [...r, data]); setNewRutinaName(''); toast.success('Rutina creada'); }
        setCreating(false);
    };
    ...
    const addEjercicio = async (rutinaId: string, ejercicioId: string) => {
        ...
        const { data, error } = await supabase
            ...
            .select('*, ejercicio:ejercicios(*)')  // tabla: ejercicios
            .single();
        if (error) { toast.error('Error al agregar ejercicio'); console.error(error.message); return; }
    ...
    const saveRutinaEjercicios = async (rutinaId: string) => {
        ...
        const updates = items.map(re =>
            supabase.from('rutina_ejercicios').update({
                series: re.series,
                            ...
                            <div style={{ padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
                                ...
                                <div>
                                    ...
                                    <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
                                        {/* columna: activa (no activo) */}
                                        {rutina.activa
                                            ? <span className="badge-active">Activa</span>
                                            : <span className="badge-expired">Inactiva</span>
                                        }
                                        {rutina.fecha_inicio && (
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    ...
                                    {expandedRutina === rutina.id
                                        ? <ChevronUp size={18} color="var(--neon)" />
                                ...
                                <div style={{ borderTop: '1px solid var(--border)', padding: '16px 20px' }}>
                                    ...
                                    <div style={{ marginBottom: 16 }}>
                                        ...
                                        <select className="input-base" defaultValue=""
                                            onChange={e => { if (e.target.value) addEjercicio(rutina.id, e.target.value); e.target.value = ''; }}
                                            style={{ cursor: 'pointer' }}>
                                            <option value="">Seleccionar ejercicio...</option>
                                            {ejerciciosMaster.map(e => (
                                    ...
                                    {/* Filas de ejercicios */}
                                    {(rutinaEjercicios[rutina.id] ?? []).map(re => (
                                        <div key={re.id} style={{
                                            display: 'grid', gridTemplateColumns: '1fr 60px 60px 70px 32px',
                                            gap: 8, alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)',
                                        }}>
                                            <div>
                                            ...
                                            <button className="btn-danger" onClick={() => removeEjercicio(rutina.id, re.id)}
                                                style={{ padding: '6px', alignSelf: 'flex-end' }}>
                                                <X size={14} />
                                            </button>
                                        </div>
                                    ))}
